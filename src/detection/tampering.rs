...
use crate::{SRegion, analysis::{copy_move::CopyMoveDetector, jpeg_analysis::JpegAnalyzer}, detection::{ConfidenceLevel, DetectedManipulation, DetectionResult, Detector, ManipulationType, splicing::SplicingDetector}, error::Result, image_utils::rgb_to_gray};
...
impl TamperingDetector {
    pub fn new() -> Self {
        Self { config: TamperingConfig::default() }
    }
    ...
    fn analyze_texture_consistency(&self, image: &RgbImage) -> Vec<(SRegion, f64)> {
        ...
        let mean_texture = block_textures
            .iter()
            .map(|(_, _, t)| t)
            .sum::<f64>() / block_textures.len() as f64;
    ...
    fn analyze_blur_consistency(&self, image: &RgbImage) -> Vec<(SRegion, f64)> {
        ...
        let mean_sharpness = block_sharpness
            .iter()
            .map(|(_, _, s)| s)
            .sum::<f64>() / block_sharpness.len() as f64;
    ...
    fn calculate_laplcaian_variance(
        &self,
        gray: &GrayImage,
        x: u32,
        y: u32,
    ...
    fn analyze_double_compression(&self, image: &DynamicImage) -> Result<Option<DetectedManipulation>> {
        ...
        if result.double_compression_likelihood > 0.6 {
            Ok(Some(DetectedManipulation { 
                ...
                evidence: vec![
                    ...
                    format!("Double compression likelihood: {:.1}%", result.double_compression_likelihood * 100.0),
                ], 
            }))
        } else {
            Ok(None)
        ...
        if self.config.detect_copy_move {
            ...
            for match_pair in &copy_move_result.matches {
                result.add_manipulation(DetectedManipulation {
                    ...
                    evidence: vec![
                        format!("Similarity: {:.1}%", match_pair.similarity * 100.0),
                    ],
                });
