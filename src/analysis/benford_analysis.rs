...
impl BenfordAnalyzer {
    ...
    pub fn analyzed(&self, image: &DynamicImage) -> Result<BenfordAnalysisResult> {
        let rgb = image.to_rgb8();
        let gray = rgb_to_gray(&rgb);
        let (width, height) = gray.dimensions();
        
        if width < self.config.block_size || height < self.config.block_size {
            return Err(crate::error::ForensicsError::ImageTooSmall(self.config.block_size));
        }
    ...
    fn compute_block_dct(&self, gray: &GrayImage, bx: u32, by: u32) -> Vec<f64> {
        ...
        for u in 0..8 {
            for v in 0..8 {
                ...
                for y in 0..8 {
                    for x in 0..8 {
                        sum += block[y][x] * (PI * (2.0 * x as f64 + 1.0) * u as f64 / 16.0).cos() * (PI * (2.0 * y as f64 + 1.0) * v as f64 / 16.0).cos();
                    }
                } 
    ...
    fn merge_two_regions(&self, a: &SRegion, b: &SRegion) -> SRegion {
        ...
        let y2 = (a.y + a.width).max(b.y + b.width);
