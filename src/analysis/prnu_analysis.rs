...
impl PrnuAnalyzer {
    ...
    pub fn analyze(&self, image: &DynamicImage) -> Result<PrnuAnalysisResult> {
        ...
        let inconsistent_regions = self.find_incosistent_regions(&correlation_map, &block_correlations);
                ...
                for dy in -radius..=radius {
                    for dx in -radius..=radius {
                        ...
                        if nx >= 0 && nx < width as i32 && ny >= 0 && ny < height as i32 {
                            ...
                            let space_weight = (-(dx*dx + dy*dy) as f64 / (2.0 * sigma_space * sigma_space)).exp();
    ...
    fn weiner_filter(&self, noise: &GrayImage, original: &GrayImage) -> GrayImage {
        ...
        for y in 0..height {
            for x in 0..width {
                let (local_mean, local_var) = self.calculate_local_stats(
                    original, x, y, block_size
                );
        ...
        for by in (0..height).step_by(block_size as usize) {
            for bx in (0..width).step_by(block_size as usize) {
                ...
                let block_var = block_values
                    ...
                    .sum::<f64>() / block_values.len() as f64;
    ...
    fn find_incosistent_regions(&self, correlation_map: &GrayImage, correlations: &[f64]) -> Vec<SRegion> {
