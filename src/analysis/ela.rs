    ...
    pub fn analyze(&self, image: &DynamicImage) -> Result<ElaResult> {
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                difference_map.put_pixel(
                    x,
                    y,
                    Luma([(gray_diff * self.amplification).min(255.0) as u8]),
                );
            }
        }
        ...
        let mean_difference =
            difference_map.iter().map(|&x| x as f64).sum::<f64>() / differences.len() as f64;
        ...
        let suspicious_regions =
            self.find_suspicious_regions(&difference_map, mean_difference + 2.0 * std_deviation);
