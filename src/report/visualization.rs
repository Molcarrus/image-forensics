...
use crate::{
    CopyMoveResult, ElaResult, FullAnalysisReport, NoiseResult, SRegion,
    detection::{DetectionResult, ManipulationType},
    error::Result,
};
...
impl Visualizer {
    ...
    pub fn overlay_heatmap(&self, original: &RgbImage, heatmap: &RgbImage) -> RgbImage {
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                let heat =
                    heatmap.get_pixel(x.min(heatmap.width() - 1), y.min(heatmap.height() - 1));
    ...
    fn draw_label(&self, image: &mut RgbImage, x: u32, y: u32, text: &str, color: Rgb<u8>) {
        let (width, height) = image.dimensions();
        let char_width = 6;
        let char_height = 8;
    ...
    pub fn create_comparison(&self, images: &[(&str, &RgbImage)]) -> RgbImage {
        ...
        let total_width = images.iter().map(|(_, img)| img.width()).sum::<u32>()
            + padding * (images.len() as u32 + 1);
    ...
    fn copy_image_to(&self, dest: &mut RgbImage, src: &RgbImage, offset_x: u32, offset_y: u32) {
...
impl ComprehensiveVisualization {
    pub fn save_all(&self, directory: &str) -> Result<()> {
        ...
        self.copy_move
            .save(format!("{}/copy_move.png", directory))?;
