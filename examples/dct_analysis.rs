...
fn main() -> Result<()> {
    ...
    println!(
        "Double compression probability: {:.1}%",
        dct_result.double_compression_probability * 100.0
    );
    if let Some(secondary) = dct_result.secondary_quality {
        println!("Secondary quality estimate: {}", secondary);
    }
    println!(
        "Histogram periodicity: {:.3}",
        dct_result.histogram_periodicity
    );
    println!(
        "Anomalous DCT regions: {}",
        dct_result.anomalous_regions.len()
    );

    dct_result.dct_energy_map.save("output/dct_energy.png")?;
    dct_result
        .block_artifact_map
        .save("output/dct_artifacts.png")?;
