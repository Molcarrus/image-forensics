...
use image_forensics::{
    AnalysisConfig, ForensicsAnalyzer, analysis::copy_move::CopyMoveDetector,
    analysis::ela::ElaAnalyzer, analysis::jpeg_analysis::JpegAnalyzer,
    analysis::noise::NoiseAnalyzer, error::Result, metadata::exif::ExifExtractor,
};
use std::env;
use std::fs;
use std::path::Path;
...
fn main() -> Result<()> {
    ...
    println!("  Results:");
    println!(
        "     Matching regions found: {}",
        copy_move_result.matches.len()
    );
    println!(
        "     Overall confidence:     {:.1}%",
        copy_move_result.confidence * 100.0
    );
    ...
    let noise_analyzer = NoiseAnalyzer::new().with_block_size(16);
    ...
    println!("  Results:");
    println!(
        "     Estimated noise level:  {:.2}",
        noise_result.estimated_noise_level
    );
    println!(
        "     Inconsistency score:    {:.2}%",
        noise_result.inconsistency_score * 100.0
    );
    println!(
        "     Anomalous regions:      {}",
        noise_result.anomalous_regions.len()
    );
    ...
    println!(
        ...
        jpeg_result.double_compression_likelihood * 100.0
    );
    ...
    match ExifExtractor::extract(image_path) {
        Ok(metadata) => {
            ...
            let metadata_json =
                serde_json::to_string_pretty(&metadata.all_tags).unwrap_or_default();
    ...
    if ela_result.max_difference > 50.0 {
        suspicion_factors.push((
            "High ELA differences",
            (ela_result.max_difference / 255.0).min(1.0),
        ));
    }
    ...
    if jpeg_result.double_compression_likelihood > 0.5 {
        suspicion_factors.push((
            "Double compression likely",
            jpeg_result.double_compression_likelihood,
        ));
    }
