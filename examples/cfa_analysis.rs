use image_forensics::{analysis::cfa_analysis::{CfaAnalyzer, CfaConfig, CfaPattern}, error::Result};
...
fn main() -> Result<()> {
    ...
    println!("Pattern confidence: {:.1}%", cfa_result.pattern_confidence * 100.0);
    println!("Consistency score: {:.1}%", cfa_result.consistency_score * 100.0);
    println!("Inconsistent regions: {}", cfa_result.inconsistent_regions.len());
    println!("Manipulation probability: {:.1}%", cfa_result.manipulation_probability * 100.0);
    ...
    cfa_result.consistency_map.save("output/cfa_consistency.png")?;
