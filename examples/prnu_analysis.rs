...
fn main() -> Result<()> {
    ...
    println!(
        "PRNU consistency score: {:.1}%",
        prnu_result.consistency_score * 100.0
    );
    println!(
        "Manipulation probability: {:.1}%",
        prnu_result.manipulation_probability * 100.0
    );
    println!(
        "Inconsistent regions: {}",
        prnu_result.inconsistent_regions.len()
    );
    println!("PRNU Statistics:");
    println!("  Mean: {:.2}", prnu_result.prnu_statistics.mean);
    println!(
        "  Standard Deviation: {:.2}",
        prnu_result.prnu_statistics.std_dev
    );
    ...
    prnu_result
        .correlation_map
        .save("output/prnu_correlation.png")?;
